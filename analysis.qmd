...
-   Who are some of the best board game designers since the 1990s?
    ...
    -   What are game categories are these prolific designers associated with?
...
This analysis is intended for anyone in PH.140.777, the general public, as well as those who are interested in board games.
...
In this section, I conduct analysis on and highlight some of the most brilliant board game designers since the 1990s.
...
::: callout-note
The game rating is averaged for all the games developed by that designer in the corresponding decades.
:::
...
As we see, the top 10 highest rated designers only developed less than 5 games. Thus, here we focus our attention on the ratings of prolific designers who have adequate number of ratings for more robust comparison.
...
top10_rated_designers <- games %>%
  separate_rows(designer, sep = ";\\s*") %>%
  dplyr::group_by(designer) %>%
  dplyr::summarise(Mean_Rating=mean(average), Number_of_Games=n()) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(designer=gsub("'","",designer)) %>%
  dplyr::filter(!is.na(designer)) %>%
  dplyr::filter(designer!="'(Uncredited)'") %>%
...
kable(top10_rated_designers, format="html",
      caption="Top 10 Highest Rated Designers") %>%
...
Evaluated by the game ratings, these games that the prolific designers produce seem to be favored by the users, as they are fairly consistent. Especially in recent years, we see an upward trend in game ratings for most of the designers shown here.
...
my_palette <- c("#0073FF", "#A4ACCB", "#7AC4F7", "#3BA500", "#FF99B0", "#FFD880", "#117799", "#90EE90", "#FFE4C4", "#A4918C")
...
In this section, I switch my focus to look at popular game categories and mechanics, using the number of 'owned' games as the metric to evaluate the popularity of a game.
...
Here, we show the details of top 15 most owned games since the 1990. Interestingly, Catan[@catan], which is a strategy-based game is the only game made in the 1990s that made the top 15.
...
In this example analysis, we explore the board game dataset to identify some of the best board game designers, based on quantity, rating, consistency, trends, owned status. We listed out a few of the best designers, including Reiner Knizia, Dean Essig, Eric M. Lang, Martin Wallace, & Matt Leacock. We also identified the top three most popular game categories and mechanics, such as card game and dice rolling, respectively. Overall, we see a positive trend of game ratings and quantity and expect the board game market to grow as projected.
